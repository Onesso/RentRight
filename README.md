...
additional dependencies;
                ...
                Pillow (Python imaging library)
                    ...
                    in order to use the above the library we need to the following dependencies in out docker image
                    1.  zlib, zlib-dev
                    2.  jpeg-dev
...
Media and static
        ...
        This are the configuration we need to add
            1.  STATIC_URL - Base static URL (eg: /static/static/)
            2.  MEDIA_URL - Base media URL (e.g /static/media)
            3.  Media_ROOT - Path to media on filesystem (eg: /vol/web/media)   this is where they are being stored
            4.  STATIC_ROOT - path to static file on filesystem (eg: /vol/web/static)

        Docker volume
            -   This is where persistent data is actually stored
            -   It is stored in the following directory; volume;
                            /vol/web - then static and media subdirectories

        Collect Static
                    -   Django provides a command to gather static files and put all static files into STATIC_ROOT
                                -   python manage.py collectstatic

        Configure project for static files
                    1.  Open the dockerfile and make the changes; add directories that will store the media files
                    2.  run build the docker image; docker-compose build
                    3.  open the docker-compose.yml and add /vol/web to make image persistant during our testing and development
                    4.  Open the settings.py file; delete the default STATIC_URL and add the STATIC_URL, MEDIA_URL, MEDIA_ROOT, STATIC_ROOT to point to the volumes and location
